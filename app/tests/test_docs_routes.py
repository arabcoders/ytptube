...
async def test_serves_nested_doc(tmp_path: Path) -> None:
    ...
    (tmp_path / "docs").mkdir()
    (tmp_path / "docs" / "features.md").write_text("features", encoding="utf-8")
    config = SimpleNamespace(app_path=str(tmp_path / "app"))
    request = SimpleNamespace(match_info={"file": "docs/features.md"}, path="/api/docs/docs/features.md")

    response = await docs.get_doc(request, config)

    assert response.status == 200
    assert response.content_type == "text/markdown"
    assert response.body == b"features"
