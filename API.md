...
**Data Fields**:
- `feature`: One of `presets`, `dl_fields`, `conditions`, `notifications`
- `action`: `create`, `update`, `delete`, `replace`
- `data`: Updated payload (object or list of objects)
...
```json
{
  ...
  "action": "update",
  "data": { "id": 1, "name": "My Webhook", "enabled": true }
}
```
...
- `history_count`: Count of completed download items

##### `active_queue` (Server → Client)
Provides the current active queue snapshot.
...
#### `item_status` (Server → Client)
Emitted after add-url attempts via WebSocket to report enqueue status.

**Data Fields**:
- `status`: Status payload from queue add operation
- `preset`: Preset name used for the request
