...
describe('useTaskDefinitions', () => {
  ...
  it('load_impersonate_targets', async () => {
    const requestSpy = spyOn(utils, 'request');
    requestSpy.mockResolvedValueOnce(
      createMockResponse({
        ok: true,
        status: 200,
        jsonData: { targets: ['chrome', 'safari'] },
      }),
    );
    const defs = useTaskDefinitions();
    await defs.loadImpersonateTargets();
    expect(defs.impersonateTargets.value).toEqual(['chrome', 'safari']);
    requestSpy.mockRestore();
  });
