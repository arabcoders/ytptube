...
describe('MatchFilterParser', () => {
  ...
  it('parse_filesize', () => {
    ...
    expect(new MatchFilterParser('filesize>=1073741824').evaluate({ filesize: 2 ** 30 })).toBe(
      true,
    );
    expect(new MatchFilterParser('filesize>=1073741824').evaluate({ filesize: 1000000 })).toBe(
      false,
    );
  });
  ...
  it('handle_operator_spaces', () => {
    ...
    expect(
      new MatchFilterParser("channel_id = 'UC-7oMv6E4Uz2tF51w5Sj49w' & uploader = 'BBC'").evaluate(
        d,
      ),
    ).toBe(true);
    expect(
      new MatchFilterParser("channel_id = 'different-id' & uploader = 'BBC'").evaluate(d),
    ).toBe(false);

    expect(
      new MatchFilterParser(
        "channel_id = 'UCfmrcEdes7yDtEISGPM1T-A' & availability = subscriber_only",
      ).evaluate(d),
    ).toBe(false);
  });
  ...
  it('reproduce_bug_report', () => {
    ...
    expect(
      new MatchFilterParser("channel_id = 'UCfmrcEdes7yDtEISGPM1T-A'").evaluate(testData),
    ).toBe(false);
    expect(new MatchFilterParser('availability = subscriber_only').evaluate(testData)).toBe(false);

    expect(
      new MatchFilterParser("channel_id = 'UC-7oMv6E4Uz2tF51w5Sj49w'").evaluate(testData),
    ).toBe(true);
  });
  ...
  it('export_filters', () => {
    ...
    const complexParser = new MatchFilterParser(
      "(filesize>1000000 & duration<600) || uploader='BBC'",
    );
    const complexExported = complexParser.export();
    expect(normalize(complexExported)).toEqual(
      normalize(['filesize>1000000&duration<600', "uploader='BBC'"]),
    );

    const testData = { filesize: 2000000, duration: 300 };
    expect(complexExported.some((filter) => new MatchFilterParser(filter).evaluate(testData))).toBe(
      true,
    );
  });
});
